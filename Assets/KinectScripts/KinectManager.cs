					...
					else if(gestureData.progress >= 0.1f)
					{
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								...
								if(ControlMouseCursor)
								{
									Vector3 vCursorPos = Camera.main.WorldToViewportPoint(HandCursor1.transform.position);
					...
					else if(gestureData.progress >= 0.1f)
					{
							...
							gestureData.progress >= 0.5f)
						{
							if(GetGestureProgress(gestureData.userId, KinectGestures.Gestures.Click) < 0.3f)
							{
								...
								if(ControlMouseCursor)
								{
									Vector3 vCursorPos = Camera.main.WorldToViewportPoint(HandCursor2.transform.position);
									MouseControl.MouseMove(vCursorPos, CalibrationText);
								}
							}
						}
